...
class DetoksTrustChainCommunity(Community):
...
async def start_nodes(num_nodes: int, timeout: int, max_peers: int):
    ...
    for i in range(num_nodes):
        ...
        ipv8 = IPv8(
            builder.finalize(), extra_communities={"DetoksTrustChainCommunity": DetoksTrustChainCommunity}
        )
        await ipv8.start()
